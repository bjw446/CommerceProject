...
public class Category {
    ...
    public void electronics() {
        ...
        for(int i = 0; i < electronics.size(); i++){
            ...
            System.out.printf("%-1d. %-15s | %,9d원 | %-1s%n", i + 1, product.getName(), product.getPrice(), product.getExplain());
        }
        ...
        switch (choice) {
            case 1, 2, 3, 4:
                ...
                System.out.printf("%-1s %-15s | %,9d원 | %-1s%n\n","선택한 상품 :", select.getName(), select.getPrice(), select.getExplain());
                break;
            case 0 :
                return;
            default :
    ...
    public void apparel() {
        ...
        for(int i = 0; i < apparel.size(); i++){
            ...
            System.out.printf("%-1d. %-23s | %,9d원 | %-1s%n", i + 1, product.getName(), product.getPrice(), product.getExplain());
        }
        System.out.printf("%-1d. %-11s%n", 0, "뒤로가기");
        ...
        switch (choice) {
            case 1, 2, 3, 4:
                ...
                System.out.printf("%-1s %-30s | %,9d원 | %-1s%n\n","선택한 상품 :", select.getName(), select.getPrice(), select.getExplain());
                break;
            case 0 :
                return;
            default :
    ...
    public void food() {
        ...
        food.add(new Product("Spam 3Kg", 35000, "340g 7개 + 80g 8개 묶음상품", 50));
        food.add(new Product("DongWon Tuna 85g", 34000, "85g 24개 묶음상품", 50));
        food.add(new Product("DaeCheon Tripe Seaweed 30g", 54000, "선물세트 30g 12개 묶음상품", 50));

        for(int i = 0; i < food.size(); i++){
            Product product = food.get(i);
            System.out.printf("%-1d. %-28s | %,9d원 | %-1s%n", i + 1, product.getName(), product.getPrice(), product.getExplain());
        }
        System.out.printf("%-1d. %-11s%n", 0, "뒤로가기");
        ...
        switch (choice) {
            case 1, 2, 3, 4:
                ...
                System.out.printf("%-1s %-28s | %,9d원 | %-1s%n\n","선택한 상품 :", select.getName(), select.getPrice(), select.getExplain());
                break;
            case 0 :
                return;
            default :
